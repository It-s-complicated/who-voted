...
| [scripts/download-data.mjs](scripts/download-data.mjs) | Berlin, Hamburg and Bremen currently have separate, exact-election source entries here. Add a new entry for their new election and any required additional sources. Other states inherit their source configuration from `stateSources`. |
| [scripts/process-data.mjs](scripts/process-data.mjs) | Reuse a parser only after verifying the actual document layout. Add and register an election-specific parser where needed. Berlin, Hamburg and Bremen are explicitly registered in the assembly block. |
